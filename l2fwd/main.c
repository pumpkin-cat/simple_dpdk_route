...
l2fwd_mac_updating(struct rte_mbuf *m, unsigned dest_portid)
{
	...
	rte_ether_addr_copy(&l2fwd_ports_eth_addr[dest_portid], &eth_hdr->src_addr);
...
l2fwd_simple_forward(struct rte_mbuf *m, unsigned portid)
{
	...
	sent = rte_eth_tx_buffer(dst_port, 0, buffer, m); // 把m放到发送队列中，如果队列满了将会一起发送所有包
	if (sent)
